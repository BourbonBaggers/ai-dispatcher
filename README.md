...
this standalone package (it involves a destructive Postgres migration and would kill the
running dispatcher mid-flight). Keep the cutover procedure in the application repository
that owns the embedded dispatcher, and only run it after this standalone service has been
installed, configured, and verified against the target repository.
