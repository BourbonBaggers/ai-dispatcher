...
scan_for_residue() {
  ...
  if ((have_rg)); then
    rg -n --hidden --no-messages \
      -g '!check-private-residue.sh' -g '!check-public-release-hygiene.sh' "$pattern" "$path"
    return
  fi

  # Use grep as a portable fallback on runners that do not ship ripgrep.
  grep -RInE --binary-files=without-match \
    --exclude='check-private-residue.sh' --exclude='check-public-release-hygiene.sh' "$pattern" "$path"
}
...
for path in "${INCLUDED_PATHS[@]}"; do
  ...
  if ((have_rg)); then
    if rg -n -P --hidden --no-messages \
      -g '!check-private-residue.sh' -g '!check-public-release-hygiene.sh' "$private_ip_pattern" "$path"; then
